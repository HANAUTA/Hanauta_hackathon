# GitHub Actions 設定手順（スマホでアプリを見られるようにする）

**チームに余裕が出てきたら**、ここに挑戦してみましょう。

今までは Chrome の中だけでアプリを動かしていましたが、この設定をすると
**スマホのブラウザからもアプリが開けるようになります**。しかも一度設定すれば、
以降はチームの誰かが `main` に push するたびに、自動で最新版に更新されます。

> 💡 これは「**PWA（Progressive Web App）**」という仕組みを使っています。
> スマホにアプリをインストールしなくても、ブラウザで開くだけで
> まるでアプリのように使える Web サイトのことです。
> App Store や Google Play を経由しないので、審査もインストールも不要 —
> URL を開くだけで動きます。
...
```
① チームの誰かが main に push する
        ↓
② GitHub のサーバー上で「GitHub Actions」が自動でアプリをビルドする
   （Flutter のコードを、ブラウザで動く形に変換する作業）
        ↓
③ ビルドされたアプリが「GitHub Pages」という無料のWeb公開場所にアップロードされる
        ↓
④ スマホのブラウザでURLを開くと、そのアプリが表示される
```

②と③をやってくれるのが GitHub Actions と GitHub Pages です。
これから設定するのは、**この一連の流れが自動で回るようにするための準備**です。
**1回だけ**やれば、あとはずっと自動で動き続けます。設定するのは代表者1人でOKです。
...
まず、②のビルドをしてくれる「GitHub Actions」という機能自体を有効にします。

このリポジトリは Fork（コピー）で作られているため、**GitHub Actions が最初は無効**になっています
（知らないリポジトリを不用意に Fork して、勝手にプログラムが動き出す事故を防ぐための GitHub の仕様です）。
...
ビルドされたアプリは、起動時に Supabase（データベース）へ接続しにいきます。
その接続先（URL と鍵）を、GitHub にも教えてあげる必要があります。

> 🤔 なぜ `.env` だけじゃダメなの？
> `.env` は自分のパソコンの中にしかない情報で、GitHub にはアップロードされません
> （わざとそう設定してあります）。なので、ビルドを行う GitHub 側にも
> 同じ情報を**別途**登録してあげる必要があるのです。「Secrets」はその専用の置き場所です。
...
>
> ⚠️ この設定ができるのは、リポジトリの Owner（＝Forkした代表者）だけです。
...
③の「公開する場所」として使うのが GitHub Pages です。これを有効にしておくことで、
ビルドが終わるたびに自動でそこへ公開されるようになります。
...
ここまでの設定が正しくできているか、実際に試してみます。グループの誰か1人がやればOKです。
...
ビルドが完了（緑のチェック ✅）したら、④で開くための URL が分かります。
以下のどちらかで見られます。
...
とすると、ホーム画面にアイコンが増えて、通常のアプリのように起動できます。

---

## 以降の使い方

一度設定してしまえば、あとは何も意識する必要はありません。
`main` にマージ → push するたびに、①〜④が自動的に繰り返されてブラウザに反映されます。
